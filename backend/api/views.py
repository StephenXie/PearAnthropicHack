...
def generate_feedback_on_task_description_view(request):
    ...
    if (
        not request.data.get("task_id")
        or not Task.objects.filter(task_id=request.data.get("task_id")).exists()
    ):
        task_id = str(uuid4())
    ...
    result = generate_feedback_on_task_description(
        task_description,
        task.conversation_data,
        task.address,
    ...
    response["response"] = result["updated_conversation"][-1]["content"][0][
        "text"
    ].strip()
